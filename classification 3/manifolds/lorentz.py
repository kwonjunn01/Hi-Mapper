...
from manifolds.utils import acosh, aacosh
import math
...
class Lorentz(LorentzOri):
    def __init__(self, k=1.0, learnable=False, max_norm=None):
        ...
        self.k = nn.Parameter(torch.tensor(k).log(),requires_grad=learnable)
        self._curv_minmax = {
            "max": math.log(k * 10),
            "min": math.log(k / 10),
        }
    ...
    def _check_vector_on_tangent(
        ...
        inner_ = lmath.inner(u, x, dim=dim)
    ...
    def dist(
        ...
        return lmath.dist(x, y, k=self.k, keepdim=keepdim, dim=dim)
    def dist_n(self, x: torch.Tensor, y: torch.Tensor, *, keepdim=False, dim=-1
    ) -> torch.Tensor:
        self.clamp_k()
        return lmath.dist_n(x, y, self.k.exp())
    def clamp_k(self):
        self.k.data = torch.clamp(self.k.data, **self._curv_minmax)
    def dist_nn(self, x: torch.Tensor, y: torch.Tensor, *, keepdim=False, dim=-1
    ) -> torch.Tensor:
        return lmath.dist_nn(x, y, self.k)
    def aacosh(self, x: torch.Tensor):
        self.clamp_k()
        eps = 1e-5
        x = -self.k.exp()*x
        x = x.clamp(min=1+eps)
    ...
    def dist0(self, x: torch.Tensor, *, dim=-1, keepdim=False) -> torch.Tensor:
        return lmath.dist0(x, k=self.k, dim=dim, keepdim=keepdim)
    ...
    def cdist(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        return lmath.cdist(x, y, k=self.k)
    ...
    def expmap(
        ...
        res = lmath.expmap(x, u, k=self.k, dim=dim)
        if project is True:
            return lmath.project(res, k=self.k, dim=dim)
        else:
            return res

    def expmap0(self, u: torch.Tensor, *, project=True, eps: float = 1e-8) -> torch.Tensor:
        self.clamp_k()
        curv = self.k.exp()
        rc_xnorm = curv**0.5 * torch.norm(u, dim=-1, keepdim=True)
        
    # Ensure numerical stability in sinh by clamping input.
        sinh_input = torch.clamp(rc_xnorm, min=eps, max=math.asinh(2**15))
        _output = torch.sinh(sinh_input) * u / torch.clamp(rc_xnorm, min=eps)
        return _output

    def logmap(self, x: torch.Tensor, y: torch.Tensor, *, dim=-1) -> torch.Tensor:
        return lmath.logmap(x, y, k=self.k, dim=dim)

    def logmap0(self, y: torch.Tensor, *, dim=-1) -> torch.Tensor:
        return lmath.logmap0(y, k=self.k, dim=dim)

    def logmap0back(self, x: torch.Tensor, *, dim=-1) -> torch.Tensor:
        return lmath.logmap0back(x, k=self.k, dim=dim)
    ...
    def inner(
        ...
        *,
        ...
        if v is None:
            v = u
        return lmath.inner(u, v, dim=dim, keepdim=keepdim)

    def inner0(self, v: torch.Tensor = None, *, keepdim=False, dim=-1,) -> torch.Tensor:
        return lmath.inner0(v, k=self.k, dim=dim, keepdim=keepdim)
    ...
    def egrad2rgrad(self, x: torch.Tensor, u: torch.Tensor, *, dim=-1) -> torch.Tensor:
        return lmath.egrad2rgrad(x, u, k=self.k, dim=dim)

    def transp(
        self, x: torch.Tensor, y: torch.Tensor, v: torch.Tensor, *, dim=-1
    ) -> torch.Tensor:
        return lmath.parallel_transport(x, y, v, k=self.k, dim=dim)

    def transp0(self, y: torch.Tensor, u: torch.Tensor, *, dim=-1) -> torch.Tensor:
        return lmath.parallel_transport0(y, u, k=self.k, dim=dim)

    def transp0back(self, x: torch.Tensor, u: torch.Tensor, *, dim=-1) -> torch.Tensor:
        return lmath.parallel_transport0back(x, u, k=self.k, dim=dim)
    ...
    def geodesic_unit(
        ...
        res = lmath.geodesic_unit(t, x, u, k=self.k)
        if project:
            return lmath.project(res, k=self.k, dim=dim)
        else:
            return res
