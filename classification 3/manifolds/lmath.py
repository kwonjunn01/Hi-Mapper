import torch
from manifolds.utils import aacosh, acosh, sqrt
...
class RiemannianGradient(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        (x,) = ctx.saved_tensors
        # x: B x d
        grad_output.narrow(-1, 0, 1).mul_(-1)
        grad_output = grad_output.addcmul(_inner(x/50, grad_output, dim=-1, keepdim=True), x/RiemannianGradient.c)
        return grad_output
...
def dist_nn(u, v, k):
    ...
    # v = v/20
    eps = 1e-5
    # u_time = torch.sqrt(1/k+torch.sum(u*u, dim=-1))
    # v_time = torch.sqrt(1/k+torch.sum(v*v, dim=-1))
    
    uv = torch.mul(u, v)
    time = uv[...,0]
    uv = -time[...,None] + uv[...,1:]
    uv = -uv/k
    uv = (uv/10).clamp(min=1+eps)
    return torch.sum(aacosh(uv), dim=-1)
def dist_n(u, v, k):
    u = u
    v = v
    eps = 1e-5
    ...
    uv = uv-time[...,None]
    uv = -uv*k
    uv = uv.clamp(min=1+eps)
    return torch.acosh(uv)/(k**0.5)
